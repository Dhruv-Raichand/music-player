...
async function getSongs(folder) {
    ...
    Array.from(document.querySelector(".songList").getElementsByTagName("li")).forEach((e, index) => {
        e.addEventListener("click", () => {
            players.removeAttribute("style", "display: none")
...
async function displayAlbums() {
    ...
    Array.from(document.getElementsByClassName("card")).forEach(e => { 
     ...
     document.getElementById('searchInput').addEventListener('input', function() {
        ...
        // Clear the card container and display filtered albums
...
// Assuming you have a play button and currentSong defined
const playButton = document.getElementById("play"); // Your play button element
...
// Assuming you have a play button and currentSong defined
const playsButton = document.getElementById("play"); // Your play button element
let currentsSong = new Audio(); // Your audio element
let currentIndex = 0; // Track the current song index

// Function to play a specific track
const playsMusic = (track) => {
    currentsSong.src = `path/to/your/songs/${track}`; // Update with your song path
    currentsSong.play();
    playButton.src = "img/pause.svg"; // Change icon to pause
};
...
const nextTrack = () => {
    currentIndex = (currentIndex + 1) % songs.length; // Loop back to the first song
    playMusic(songs[currentIndex]);
};

// Function to play the previous track
const previousTrack = () => {
    currentIndex = (currentIndex - 1 + songslength) % songs.length; // Loop to the last song
    playMusic(songs[currentIndex]);
};

// Event listener for key mappings
document.addEventListener("keydown", (event) => {
    switch (event.code) {
        case "ArrowRight":
            event.preventDefault(); // Prevent default scrolling behavior
            nextTrack(); // Call the function to play the next track
            break;
        case "ArrowLeft":
            event.preventDefault(); // Prevent default scrolling behavior
            previousTrack(); // Call the function to play the previous track
            break;
    }
});
